...
func main() {
	...
	http.HandleFunc("GET /get-user/{id}", middleware.AuthMiddleWare(handlers.GetUserById))
	http.HandleFunc("GET /me", middleware.AuthMiddleWare(handlers.GetMe))
	http.HandleFunc("DELETE /delete-users", middleware.AuthMiddleWare(handlers.DeleteAllUsers))
	http.HandleFunc("DELETE /delete-user/{id}", middleware.AuthMiddleWare(handlers.DeleteUserById))
